		...
		<table class="w3-table">
			...
			<c:forEach var="article" items="${messagelist}">
				...
				<tr>
					...
					<c:if test="${group !='3'}">
					<td><a href="messagecontent?num=${article.num}&group=${group}"><font color="#A6A6A6">${article.receivemessage}</font></a></td>
					</c:if>
					<c:if test="${group =='3'}">
				...
				<tr>
					...
					<c:if test="${group =='3'}">
					<td>${article.userinfo2}</td>
					</c:if>
					<c:if test="${group !='3'}">
					<td><a href="messagecontent?num=${article.num}&group=${group}">${article.receivemessage}</a></td>
					</c:if>
					<c:if test="${group =='3'}">
